...
export class AccessTokenGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!token) {
      throw new UnauthorizedException('Please provide access token (กรุณาส่ง access token)');
    }
    ...
    try {
      ...
      throw new UnauthorizedException('Invalid or expired access token (access token ไม่ถูกต้องหรือหมดอายุ)');
    }
  }
